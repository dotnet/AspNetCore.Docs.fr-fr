...
[Afficher ou télécharger l’exemple de code](https://github.com/dotnet/AspNetCore.Docs/tree/main/aspnetcore/blazor/common/samples/) ([procédure de téléchargement](xref:index#how-to-download-a-sample))
...
Une valeur par défaut peut être assignée aux paramètres du composant :
...
Pour plus d’informations sur la façon dont un <xref:Microsoft.AspNetCore.Components.RenderFragment> peut être utilisé comme modèle pour Razor l’interface utilisateur du composant, consultez les articles suivants :
...
Lors du rendu du composant, le `loginDialog` champ est rempli avec l' `CustomLoginDialog` instance du composant enfant. Vous pouvez ensuite appeler des méthodes .NET sur l’instance du composant.
...
Un composant enfant reçoit de nouvelles valeurs de paramètre qui, éventuellement, remplacent les valeurs existantes lors du rerendu du composant parent. Le remplacement accidentel de valeurs de paramètre dans un composant enfant se produit souvent lors du développement du composant avec un ou plusieurs paramètres liés aux données et le développeur écrit directement dans un paramètre de l’enfant :
...
## <a name="razor-templates"></a>Razor ceux
