...
<span data-ttu-id="95c2f-105">Azure Active Directory (AAD) fournit plusieurs approches d’autorisation qui peuvent être combinées avec ASP.NET Core Identity :</span><span class="sxs-lookup"><span data-stu-id="95c2f-105">Azure Active Directory (AAD) provides several authorization approaches that can be combined with ASP.NET Core Identity:</span></span>
...
* <span data-ttu-id="95c2f-110">Rôles</span><span class="sxs-lookup"><span data-stu-id="95c2f-110">Roles</span></span>
  ...
  * <span data-ttu-id="95c2f-112">Rôles définis par l’utilisateur</span><span class="sxs-lookup"><span data-stu-id="95c2f-112">User-defined roles</span></span>

<span data-ttu-id="95c2f-113">Les instructions de cet article s’appliquent aux Blazor WebAssembly scénarios de déploiement AAD décrits dans les rubriques suivantes :</span><span class="sxs-lookup"><span data-stu-id="95c2f-113">The guidance in this article applies to the Blazor WebAssembly AAD deployment scenarios described in the following topics:</span></span>
...
<span data-ttu-id="95c2f-119">Pour autoriser les appels API Graph, attribuez à l’application autonome ou à *`Client`* une solution hébergée l' Blazor une des [autorisations API Graph suivantes (étendues)](/graph/permissions-reference) dans le portail Azure :</span><span class="sxs-lookup"><span data-stu-id="95c2f-119">To permit Graph API calls, give the standalone or *`Client`* app of a hosted Blazor solution any of the following [Graph API permissions (scopes)](/graph/permissions-reference) in the Azure portal:</span></span>
...
* [<span data-ttu-id="95c2f-124">Rôles utilisant les groupes de sécurité Azure AD</span><span class="sxs-lookup"><span data-stu-id="95c2f-124">Roles using Azure AD security groups</span></span>](/azure/architecture/multitenant-identity/app-roles#roles-using-azure-ad-security-groups)
...
<span data-ttu-id="95c2f-140">Ajoutez une référence de package à l’application autonome ou à l' *`Client`* application d’une solution hébergée Blazor pour [`Microsoft.Graph`](https://www.nuget.org/packages/Microsoft.Graph) .</span><span class="sxs-lookup"><span data-stu-id="95c2f-140">Add a package reference to the standalone app or *`Client`* app of a hosted Blazor solution for [`Microsoft.Graph`](https://www.nuget.org/packages/Microsoft.Graph).</span></span>
...
<span data-ttu-id="95c2f-171">Inscrire la fabrique dans `Program.Main` ( `Program.cs` ) de l’application autonome ou *`Client`* de l’application d’une solution hébergée Blazor .</span><span class="sxs-lookup"><span data-stu-id="95c2f-171">Register the factory in `Program.Main` (`Program.cs`) of the standalone app or *`Client`* app of a hosted Blazor solution.</span></span> <span data-ttu-id="95c2f-172">Consentement à l' `Directory.Read.All` étendue comme étendue supplémentaire pour l’application :</span><span class="sxs-lookup"><span data-stu-id="95c2f-172">Consent to the `Directory.Read.All` scope as an additional scope for the app:</span></span>
...
<span data-ttu-id="95c2f-183">Outre l’autorisation des utilisateurs dans l’application webassembly côté client pour accéder aux pages et aux ressources, l’API serveur peut autoriser les utilisateurs à accéder aux points de terminaison d’API sécurisés.</span><span class="sxs-lookup"><span data-stu-id="95c2f-183">In addition to authorizing users in the client-side WebAssembly app to access pages and resources, the server API can authorize users for access to secure API endpoints.</span></span> <span data-ttu-id="95c2f-184">Une fois que l’application *serveur* a validé le jeton d’accès de l’utilisateur :</span><span class="sxs-lookup"><span data-stu-id="95c2f-184">After the *Server* app validates the user's access token:</span></span>
...
### <a name="azure-configuration"></a><span data-ttu-id="95c2f-197">Configuration Azure</span><span class="sxs-lookup"><span data-stu-id="95c2f-197">Azure configuration</span></span>
...
<span data-ttu-id="95c2f-203">Dans le fichier de paramètres d’application ( `appsettings.json` ou `appsettings.Production.json` ), créez une `ClientSecret` entrée avec la clé secrète client de l’application *serveur* à partir de la portail Azure :</span><span class="sxs-lookup"><span data-stu-id="95c2f-203">In the app settings file (`appsettings.json` or `appsettings.Production.json`), create a `ClientSecret` entry with the *Server* app's client secret from the Azure portal:</span></span>
...
* <span data-ttu-id="95c2f-223">[Microsoft. Identity . Client](https://www.nuget.org/packages/Microsoft.Identity.Client)</span><span class="sxs-lookup"><span data-stu-id="95c2f-223">[Microsoft.Identity.Client](https://www.nuget.org/packages/Microsoft.Identity.Client)</span></span>
...
* <span data-ttu-id="95c2f-242">[Microsoft. Identity Model. clients. ActiveDirectory](https://www.nuget.org/packages?q=Microsoft.IdentityModel.Clients.ActiveDirectory)</span><span class="sxs-lookup"><span data-stu-id="95c2f-242">[Microsoft.IdentityModel.Clients.ActiveDirectory](https://www.nuget.org/packages?q=Microsoft.IdentityModel.Clients.ActiveDirectory)</span></span>
...
<span data-ttu-id="95c2f-272">Une application inscrite à AAD peut également être configurée pour utiliser des rôles définis par l’utilisateur.</span><span class="sxs-lookup"><span data-stu-id="95c2f-272">An AAD-registered app can also be configured to use user-defined roles.</span></span>
...
<span data-ttu-id="95c2f-280">La `CustomUserFactory` section des [groupes définis par l’utilisateur et des rôles d’administrateur AAD](#user-defined-groups-and-administrator-roles) est configurée pour agir sur une `roles` revendication avec une valeur de tableau JSON.</span><span class="sxs-lookup"><span data-stu-id="95c2f-280">The `CustomUserFactory` shown in the [User-defined groups and AAD Administrator Roles](#user-defined-groups-and-administrator-roles) section is set up to act on a `roles` claim with a JSON array value.</span></span> <span data-ttu-id="95c2f-281">Ajoutez et inscrivez `CustomUserFactory` dans l’application autonome ou dans l' *`Client`* application d’une Blazor solution hébergée, comme indiqué dans la section [groupes définis par l’utilisateur et rôles d’administrateur AAD](#user-defined-groups-and-administrator-roles) .</span><span class="sxs-lookup"><span data-stu-id="95c2f-281">Add and register the `CustomUserFactory` in the standalone app or *`Client`* app of a hosted Blazor solution as shown in the [User-defined groups and AAD Administrator Roles](#user-defined-groups-and-administrator-roles) section.</span></span> <span data-ttu-id="95c2f-282">Il n’est pas nécessaire de fournir du code pour supprimer la revendication d’origine `roles` , car elle est automatiquement supprimée par l’infrastructure.</span><span class="sxs-lookup"><span data-stu-id="95c2f-282">There's no need to provide code to remove the original `roles` claim because it's automatically removed by the framework.</span></span>

<span data-ttu-id="95c2f-283">Dans `Program.Main` l’application autonome ou l' *`Client`* application d’une solution hébergée Blazor , spécifiez la revendication nommée « `role` » comme revendication de rôle :</span><span class="sxs-lookup"><span data-stu-id="95c2f-283">In `Program.Main` of the standalone app or *`Client`* app of a hosted Blazor solution, specify the claim named "`role`" as the role claim:</span></span>
...
<span data-ttu-id="95c2f-294">Rôle administrateur AAD</span><span class="sxs-lookup"><span data-stu-id="95c2f-294">AAD Administrator Role</span></span> | <span data-ttu-id="95c2f-295">ID de l'objet</span><span class="sxs-lookup"><span data-stu-id="95c2f-295">Object ID</span></span>
--- | ---
...
<span data-ttu-id="95c2f-344">Inviteur d’invités</span><span class="sxs-lookup"><span data-stu-id="95c2f-344">Guest inviter</span></span> | <span data-ttu-id="95c2f-345">4c730a1d-cc22-44af-8f9f-4eec635c7502</span><span class="sxs-lookup"><span data-stu-id="95c2f-345">4c730a1d-cc22-44af-8f9f-4eec635c7502</span></span>
<span data-ttu-id="95c2f-346">Administrateur du support technique</span><span class="sxs-lookup"><span data-stu-id="95c2f-346">Helpdesk administrator</span></span> | <span data-ttu-id="95c2f-347">108678c8-6628-44e1-8d01-caf598a6a5f5</span><span class="sxs-lookup"><span data-stu-id="95c2f-347">108678c8-6628-44e1-8d01-caf598a6a5f5</span></span>
<span data-ttu-id="95c2f-348">Administrateur Intune</span><span class="sxs-lookup"><span data-stu-id="95c2f-348">Intune administrator</span></span> | <span data-ttu-id="95c2f-349">79950741-23fa-4189-b2cb-46640601c497</span><span class="sxs-lookup"><span data-stu-id="95c2f-349">79950741-23fa-4189-b2cb-46640601c497</span></span>
<span data-ttu-id="95c2f-350">Administrateur Kaizala</span><span class="sxs-lookup"><span data-stu-id="95c2f-350">Kaizala administrator</span></span> | <span data-ttu-id="95c2f-351">d6322af2-48e7-42e0-8c68-0bbe31af3412</span><span class="sxs-lookup"><span data-stu-id="95c2f-351">d6322af2-48e7-42e0-8c68-0bbe31af3412</span></span>
<span data-ttu-id="95c2f-352">Administrateur de licence</span><span class="sxs-lookup"><span data-stu-id="95c2f-352">License administrator</span></span> | <span data-ttu-id="95c2f-353">3355458a-e423-44bf-8b98-4ac5e572cea5</span><span class="sxs-lookup"><span data-stu-id="95c2f-353">3355458a-e423-44bf-8b98-4ac5e572cea5</span></span>
<span data-ttu-id="95c2f-354">Lecteur de confidentialité du Centre de messages</span><span class="sxs-lookup"><span data-stu-id="95c2f-354">Message center privacy reader</span></span> | <span data-ttu-id="95c2f-355">6395db95-9fb8-42b9-b1ed-30a2405eee6f</span><span class="sxs-lookup"><span data-stu-id="95c2f-355">6395db95-9fb8-42b9-b1ed-30a2405eee6f</span></span>
<span data-ttu-id="95c2f-356">Lecteur du Centre de messages</span><span class="sxs-lookup"><span data-stu-id="95c2f-356">Message center reader</span></span> | <span data-ttu-id="95c2f-357">fd5d37b8-4e24-434b-9e63-70ed3b759a16</span><span class="sxs-lookup"><span data-stu-id="95c2f-357">fd5d37b8-4e24-434b-9e63-70ed3b759a16</span></span>
<span data-ttu-id="95c2f-358">Administrateur d’applications Office</span><span class="sxs-lookup"><span data-stu-id="95c2f-358">Office apps administrator</span></span> | <span data-ttu-id="95c2f-359">5f3870cd-b042-4f93-86d7-c9d77c664dc7</span><span class="sxs-lookup"><span data-stu-id="95c2f-359">5f3870cd-b042-4f93-86d7-c9d77c664dc7</span></span>
<span data-ttu-id="95c2f-360">Administrateur de mots de passe</span><span class="sxs-lookup"><span data-stu-id="95c2f-360">Password administrator</span></span> | <span data-ttu-id="95c2f-361">466e48b7-5d66-4ae5-8911-1a118de74941</span><span class="sxs-lookup"><span data-stu-id="95c2f-361">466e48b7-5d66-4ae5-8911-1a118de74941</span></span>
<span data-ttu-id="95c2f-362">Administrateur Power BI</span><span class="sxs-lookup"><span data-stu-id="95c2f-362">Power BI administrator</span></span> | <span data-ttu-id="95c2f-363">984e83b8-8337-4255-91a1-acb663175ab4</span><span class="sxs-lookup"><span data-stu-id="95c2f-363">984e83b8-8337-4255-91a1-acb663175ab4</span></span>
<span data-ttu-id="95c2f-364">Administrateur de plateforme Power</span><span class="sxs-lookup"><span data-stu-id="95c2f-364">Power platform administrator</span></span> | <span data-ttu-id="95c2f-365">76d6f95e-9a15-4d7d-8d21-00de00faf9fd</span><span class="sxs-lookup"><span data-stu-id="95c2f-365">76d6f95e-9a15-4d7d-8d21-00de00faf9fd</span></span>
<span data-ttu-id="95c2f-366">Administrateur d’authentification privilégié</span><span class="sxs-lookup"><span data-stu-id="95c2f-366">Privileged authentication administrator</span></span> | <span data-ttu-id="95c2f-367">0829f731-b46d-419F-9742-aeb122367d11</span><span class="sxs-lookup"><span data-stu-id="95c2f-367">0829f731-b46d-419f-9742-aeb122367d11</span></span>
<span data-ttu-id="95c2f-368">Administrateur de rôle privilégié</span><span class="sxs-lookup"><span data-stu-id="95c2f-368">Privileged role administrator</span></span> | <span data-ttu-id="95c2f-369">f20a725a-d1c8-4107-83ea-1171c97d00c7</span><span class="sxs-lookup"><span data-stu-id="95c2f-369">f20a725a-d1c8-4107-83ea-1171c97d00c7</span></span>
<span data-ttu-id="95c2f-370">Lecteur de rapports</span><span class="sxs-lookup"><span data-stu-id="95c2f-370">Reports reader</span></span> | <span data-ttu-id="95c2f-371">54635450-e8ed-4f2d-9632-07db2517b4de</span><span class="sxs-lookup"><span data-stu-id="95c2f-371">54635450-e8ed-4f2d-9632-07db2517b4de</span></span>
<span data-ttu-id="95c2f-372">Administrateur de recherche</span><span class="sxs-lookup"><span data-stu-id="95c2f-372">Search administrator</span></span> | <span data-ttu-id="95c2f-373">c770a2f1-c9ba-4e60-9176-9f52b1eb1a31</span><span class="sxs-lookup"><span data-stu-id="95c2f-373">c770a2f1-c9ba-4e60-9176-9f52b1eb1a31</span></span>
<span data-ttu-id="95c2f-374">Éditeur de recherche</span><span class="sxs-lookup"><span data-stu-id="95c2f-374">Search editor</span></span> | <span data-ttu-id="95c2f-375">6a6858c6-5f0d-44ac-87c7-0190fbedd271</span><span class="sxs-lookup"><span data-stu-id="95c2f-375">6a6858c6-5f0d-44ac-87c7-0190fbedd271</span></span>
<span data-ttu-id="95c2f-376">Administrateur de sécurité</span><span class="sxs-lookup"><span data-stu-id="95c2f-376">Security administrator</span></span> | <span data-ttu-id="95c2f-377">20fa50e3-6531-44d8-bd39-b251420568ad</span><span class="sxs-lookup"><span data-stu-id="95c2f-377">20fa50e3-6531-44d8-bd39-b251420568ad</span></span>
<span data-ttu-id="95c2f-378">Opérateur de sécurité</span><span class="sxs-lookup"><span data-stu-id="95c2f-378">Security operator</span></span> | <span data-ttu-id="95c2f-379">43aae017-8e51-4188-91ab-e6debd572800</span><span class="sxs-lookup"><span data-stu-id="95c2f-379">43aae017-8e51-4188-91ab-e6debd572800</span></span>
<span data-ttu-id="95c2f-380">Lecteur de sécurité</span><span class="sxs-lookup"><span data-stu-id="95c2f-380">Security reader</span></span> | <span data-ttu-id="95c2f-381">45035cd3-fd97-4250-8197-3a53d3562d9b</span><span class="sxs-lookup"><span data-stu-id="95c2f-381">45035cd3-fd97-4250-8197-3a53d3562d9b</span></span>
<span data-ttu-id="95c2f-382">Administrateur de support de service</span><span class="sxs-lookup"><span data-stu-id="95c2f-382">Service support administrator</span></span> | <span data-ttu-id="95c2f-383">2c92cf45-c914-48f8-9bf9-fc14b28818ab</span><span class="sxs-lookup"><span data-stu-id="95c2f-383">2c92cf45-c914-48f8-9bf9-fc14b28818ab</span></span>
<span data-ttu-id="95c2f-384">Administrateur SharePoint</span><span class="sxs-lookup"><span data-stu-id="95c2f-384">SharePoint administrator</span></span> | <span data-ttu-id="95c2f-385">e1c32229-875e-461d-ae24-3cb99116e86c</span><span class="sxs-lookup"><span data-stu-id="95c2f-385">e1c32229-875e-461d-ae24-3cb99116e86c</span></span>
<span data-ttu-id="95c2f-386">Administrateur Skype Entreprise</span><span class="sxs-lookup"><span data-stu-id="95c2f-386">Skype for Business administrator</span></span> | <span data-ttu-id="95c2f-387">0a8cee12-e21d-43ef-abd9-f1ea85710e30</span><span class="sxs-lookup"><span data-stu-id="95c2f-387">0a8cee12-e21d-43ef-abd9-f1ea85710e30</span></span>
<span data-ttu-id="95c2f-388">Administrateur des communications Teams</span><span class="sxs-lookup"><span data-stu-id="95c2f-388">Teams Communications Administrator</span></span> | <span data-ttu-id="95c2f-389">2393e455-6e13-4743-9f52-63fcec2b6a9c</span><span class="sxs-lookup"><span data-stu-id="95c2f-389">2393e455-6e13-4743-9f52-63fcec2b6a9c</span></span>
<span data-ttu-id="95c2f-390">Ingénieur de support des communications Teams</span><span class="sxs-lookup"><span data-stu-id="95c2f-390">Teams Communications Support Engineer</span></span> | <span data-ttu-id="95c2f-391">802dd94e-d717-46f6-af98-b9167071e9fc</span><span class="sxs-lookup"><span data-stu-id="95c2f-391">802dd94e-d717-46f6-af98-b9167071e9fc</span></span>
<span data-ttu-id="95c2f-392">Spécialiste des communications des équipes</span><span class="sxs-lookup"><span data-stu-id="95c2f-392">Teams Communications Specialist</span></span> | <span data-ttu-id="95c2f-393">ef547281-cf46-4cc6-bcaa-f5eac3f030c9</span><span class="sxs-lookup"><span data-stu-id="95c2f-393">ef547281-cf46-4cc6-bcaa-f5eac3f030c9</span></span>
<span data-ttu-id="95c2f-394">Administrateur du service Teams</span><span class="sxs-lookup"><span data-stu-id="95c2f-394">Teams Service Administrator</span></span> | <span data-ttu-id="95c2f-395">8846a0be-197b-443a-b13c-11192691fa24</span><span class="sxs-lookup"><span data-stu-id="95c2f-395">8846a0be-197b-443a-b13c-11192691fa24</span></span>
<span data-ttu-id="95c2f-396">Administrateur d’utilisateurs</span><span class="sxs-lookup"><span data-stu-id="95c2f-396">User administrator</span></span> | <span data-ttu-id="95c2f-397">1f6eed58-7dd3-460b-a298-666f975427a1</span><span class="sxs-lookup"><span data-stu-id="95c2f-397">1f6eed58-7dd3-460b-a298-666f975427a1</span></span>

## <a name="additional-resources"></a><span data-ttu-id="95c2f-398">Ressources supplémentaires</span><span class="sxs-lookup"><span data-stu-id="95c2f-398">Additional resources</span></span>
