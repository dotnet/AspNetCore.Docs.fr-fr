...
---
# <a name="debug-aspnet-core-no-locblazor-webassembly"></a><span data-ttu-id="cd18a-103">ASP.NET Core de débogage :::no-loc(Blazor WebAssembly):::</span><span class="sxs-lookup"><span data-stu-id="cd18a-103">Debug ASP.NET Core :::no-loc(Blazor WebAssembly):::</span></span>
...
<span data-ttu-id="cd18a-105">:::no-loc(Blazor WebAssembly)::: les applications peuvent être déboguées à l’aide des outils de développement de navigateur dans les navigateurs basés sur le chrome (Edge/chrome).</span><span class="sxs-lookup"><span data-stu-id="cd18a-105">:::no-loc(Blazor WebAssembly)::: apps can be debugged using the browser dev tools in Chromium-based browsers (Edge/Chrome).</span></span> <span data-ttu-id="cd18a-106">Vous pouvez également déboguer votre application à l’aide des environnements de développement intégré (IDE) suivants :</span><span class="sxs-lookup"><span data-stu-id="cd18a-106">You can also debug your app using the following integrated development environments (IDEs):</span></span>
...
<span data-ttu-id="cd18a-134">Pour activer le débogage d’une :::no-loc(Blazor WebAssembly)::: application existante, mettez à jour le `launchSettings.json` fichier dans le projet de démarrage pour inclure la `inspectUri` propriété suivante dans chaque profil de lancement :</span><span class="sxs-lookup"><span data-stu-id="cd18a-134">To enable debugging for an existing :::no-loc(Blazor WebAssembly)::: app, update the `launchSettings.json` file in the startup project to include the following `inspectUri` property in each launch profile:</span></span>
...
* <span data-ttu-id="cd18a-137">Permet à l’IDE de détecter que l’application est une :::no-loc(Blazor WebAssembly)::: application.</span><span class="sxs-lookup"><span data-stu-id="cd18a-137">Enables the IDE to detect that the app is a :::no-loc(Blazor WebAssembly)::: app.</span></span>
* <span data-ttu-id="cd18a-138">Indique à l’infrastructure de débogage de script de se connecter au navigateur via le :::no-loc(Blazor)::: proxy de débogage de.</span><span class="sxs-lookup"><span data-stu-id="cd18a-138">Instructs the script debugging infrastructure to connect to the browser through :::no-loc(Blazor):::'s debugging proxy.</span></span>
...
<span data-ttu-id="cd18a-141">Pour déboguer une :::no-loc(Blazor WebAssembly)::: application dans Visual Studio :</span><span class="sxs-lookup"><span data-stu-id="cd18a-141">To debug a :::no-loc(Blazor WebAssembly)::: app in Visual Studio:</span></span>

1. <span data-ttu-id="cd18a-142">Créez une nouvelle ASP.NET Core application hébergée :::no-loc(Blazor WebAssembly)::: .</span><span class="sxs-lookup"><span data-stu-id="cd18a-142">Create a new ASP.NET Core hosted :::no-loc(Blazor WebAssembly)::: app.</span></span>
...
<span data-ttu-id="cd18a-150">Lors du débogage d’une :::no-loc(Blazor WebAssembly)::: application, vous pouvez également déboguer le code serveur :</span><span class="sxs-lookup"><span data-stu-id="cd18a-150">While debugging a :::no-loc(Blazor WebAssembly)::: app, you can also debug server code:</span></span>
...
<h2 id="vscode"><span data-ttu-id="cd18a-172">Déboguer autonome :::no-loc(Blazor WebAssembly):::</span><span class="sxs-lookup"><span data-stu-id="cd18a-172">Debug standalone :::no-loc(Blazor WebAssembly):::</span></span></h2>

1. <span data-ttu-id="cd18a-173">Ouvrez l' :::no-loc(Blazor WebAssembly)::: application autonome dans vs code.</span><span class="sxs-lookup"><span data-stu-id="cd18a-173">Open the standalone :::no-loc(Blazor WebAssembly)::: app in VS Code.</span></span>
   ...
   > <span data-ttu-id="cd18a-175">Une configuration supplémentaire est requise pour déboguer des :::no-loc(Blazor WebAssembly)::: applications.</span><span class="sxs-lookup"><span data-stu-id="cd18a-175">Additional setup is required to debug :::no-loc(Blazor WebAssembly)::: applications.</span></span>
...
1. <span data-ttu-id="cd18a-188">Quand vous y êtes invité, sélectionnez l’option de **:::no-loc(Blazor WebAssembly)::: débogage** pour démarrer le débogage.</span><span class="sxs-lookup"><span data-stu-id="cd18a-188">When prompted, select the **:::no-loc(Blazor WebAssembly)::: Debug** option to start debugging.</span></span>
...
## <a name="debug-hosted-no-locblazor-webassembly"></a><span data-ttu-id="cd18a-194">Débogage hébergé :::no-loc(Blazor WebAssembly):::</span><span class="sxs-lookup"><span data-stu-id="cd18a-194">Debug hosted :::no-loc(Blazor WebAssembly):::</span></span>

1. <span data-ttu-id="cd18a-195">Ouvrez le :::no-loc(Blazor WebAssembly)::: dossier de solution de l’application hébergée dans vs code.</span><span class="sxs-lookup"><span data-stu-id="cd18a-195">Open the hosted :::no-loc(Blazor WebAssembly)::: app's solution folder in VS Code.</span></span>
...
<span data-ttu-id="cd18a-203">Pour attacher une application en cours d’exécution :::no-loc(Blazor)::: , créez un `launch.json` fichier avec la configuration suivante :</span><span class="sxs-lookup"><span data-stu-id="cd18a-203">To attach to a running :::no-loc(Blazor)::: app, create a `launch.json` file with the following configuration:</span></span>
...
| `hosted`  | <span data-ttu-id="cd18a-218">Doit avoir la valeur en `true` cas de lancement et de débogage d’une application hébergée :::no-loc(Blazor WebAssembly)::: .</span><span class="sxs-lookup"><span data-stu-id="cd18a-218">Must be set to `true` if launching and debugging a hosted :::no-loc(Blazor WebAssembly)::: app.</span></span> |
...
### <a name="launch-and-debug-a-standalone-no-locblazor-webassembly-app"></a><span data-ttu-id="cd18a-230">Lancer et déboguer une :::no-loc(Blazor WebAssembly)::: application autonome</span><span class="sxs-lookup"><span data-stu-id="cd18a-230">Launch and debug a standalone :::no-loc(Blazor WebAssembly)::: app</span></span>
...
### <a name="launch-and-debug-a-hosted-no-locblazor-webassembly-app-with-microsoft-edge"></a><span data-ttu-id="cd18a-232">Lancer et déboguer une application hébergée :::no-loc(Blazor WebAssembly)::: avec Microsoft Edge</span><span class="sxs-lookup"><span data-stu-id="cd18a-232">Launch and debug a hosted :::no-loc(Blazor WebAssembly)::: app with Microsoft Edge</span></span>
...
<span data-ttu-id="cd18a-239">Pour déboguer une :::no-loc(Blazor WebAssembly)::: application dans Visual Studio pour Mac :</span><span class="sxs-lookup"><span data-stu-id="cd18a-239">To debug a :::no-loc(Blazor WebAssembly)::: app in Visual Studio for Mac:</span></span>

1. <span data-ttu-id="cd18a-240">Créez une nouvelle ASP.NET Core application hébergée :::no-loc(Blazor WebAssembly)::: .</span><span class="sxs-lookup"><span data-stu-id="cd18a-240">Create a new ASP.NET Core hosted :::no-loc(Blazor WebAssembly)::: app.</span></span>
...
<span data-ttu-id="cd18a-249">Lors du débogage d’une :::no-loc(Blazor WebAssembly)::: application, vous pouvez également déboguer le code serveur :</span><span class="sxs-lookup"><span data-stu-id="cd18a-249">While debugging a :::no-loc(Blazor WebAssembly)::: app, you can also debug server code:</span></span>
...
<span data-ttu-id="cd18a-271">:::no-loc(Blazor)::: fournit un proxy de débogage qui implémente le [protocole chrome devtools](https://chromedevtools.github.io/devtools-protocol/) et augmente le protocole avec. Informations spécifiques à .net.</span><span class="sxs-lookup"><span data-stu-id="cd18a-271">:::no-loc(Blazor)::: provides a debugging proxy that implements the [Chrome DevTools Protocol](https://chromedevtools.github.io/devtools-protocol/) and augments the protocol with .NET-specific information.</span></span> <span data-ttu-id="cd18a-272">Quand le raccourci clavier de débogage est enfoncé, :::no-loc(Blazor)::: pointe le devtools chrome au niveau du proxy.</span><span class="sxs-lookup"><span data-stu-id="cd18a-272">When debugging keyboard shortcut is pressed, :::no-loc(Blazor)::: points the Chrome DevTools at the proxy.</span></span> <span data-ttu-id="cd18a-273">Le proxy se connecte à la fenêtre du navigateur que vous cherchez à déboguer (par conséquent, il est nécessaire d’activer le débogage distant).</span><span class="sxs-lookup"><span data-stu-id="cd18a-273">The proxy connects to the browser window you're seeking to debug (hence the need to enable remote debugging).</span></span>
...
<span data-ttu-id="cd18a-275">Les mappages de source de navigateur permettent au navigateur de mapper les fichiers compilés à leurs fichiers sources d’origine et sont couramment utilisés pour le débogage côté client.</span><span class="sxs-lookup"><span data-stu-id="cd18a-275">Browser source maps allow the browser to map compiled files back to their original source files and are commonly used for client-side debugging.</span></span> <span data-ttu-id="cd18a-276">Toutefois, :::no-loc(Blazor)::: ne mappe actuellement pas C# directement à JavaScript/WASM.</span><span class="sxs-lookup"><span data-stu-id="cd18a-276">However, :::no-loc(Blazor)::: doesn't currently map C# directly to JavaScript/WASM.</span></span> <span data-ttu-id="cd18a-277">Au lieu de cela, :::no-loc(Blazor)::: fait l’interprétation du langage intermédiaire dans le navigateur, les mappages de source ne sont donc pas pertinents.</span><span class="sxs-lookup"><span data-stu-id="cd18a-277">Instead, :::no-loc(Blazor)::: does IL interpretation within the browser, so source maps aren't relevant.</span></span>
...
<span data-ttu-id="cd18a-306">Le :::no-loc(Blazor)::: proxy de débogage du Framework prend un peu de temps, il est donc possible que les points d’arrêt dans la [ `OnInitialized{Async}` méthode Lifecycle](xref:blazor/components/lifecycle#component-initialization-methods) ne soient pas atteints.</span><span class="sxs-lookup"><span data-stu-id="cd18a-306">The :::no-loc(Blazor)::: framework's debugging proxy takes a short time to launch, so breakpoints in the [`OnInitialized{Async}` lifecycle method](xref:blazor/components/lifecycle#component-initialization-methods) might not be hit.</span></span> <span data-ttu-id="cd18a-307">Nous vous recommandons d’ajouter un délai au début du corps de la méthode pour permettre au proxy de débogage de se lancer avant que le point d’arrêt ne soit atteint.</span><span class="sxs-lookup"><span data-stu-id="cd18a-307">We recommend adding a delay at the start of the method body to give the debug proxy some time to launch before the breakpoint is hit.</span></span> <span data-ttu-id="cd18a-308">Vous pouvez inclure le délai en fonction d’une [ `if` directive de compilateur](/dotnet/csharp/language-reference/preprocessor-directives/preprocessor-if) pour vous assurer que le délai n’est pas présent pour une version Release de l’application.</span><span class="sxs-lookup"><span data-stu-id="cd18a-308">You can include the delay based on an [`if` compiler directive](/dotnet/csharp/language-reference/preprocessor-directives/preprocessor-if) to ensure that the delay isn't present for a release build of the app.</span></span>
