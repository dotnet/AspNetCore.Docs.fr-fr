...
1. Dans `wwwroot/index.html` :

   * Ajoutez un `<link>` élément à l' `<head>` élément pour les styles délimités.
   * Modifiez l’élément qui charge le `App` composant.
...
1. Dans `Shared/MainLayout.razor` , placez tout le contenu d’un `<div>` élément avec un `id` de `page` :
...
1. Le dernier fichier de base `wwwroot/css/app.css` comprend les styles suivants. Supprimez les styles supplémentaires en laissant les styles suivants et ceux que vous avez ajoutés à l’application.

    `wwwroot/css/app.css` (styles de base sans les styles ajoutés par le développeur) :
...
* Dans le panneau Portail Azure l' **authentification** de l’inscription de l’application, définissez la configuration de la plateforme sur **une application à page unique** avec l’URI de redirection de l’application.

* Dans le panneau **authentification** , désactivez l' **octroi implicite** pour les **jetons d’accès** et les **jetons d’ID**.
...
### <a name="hosted-no-locblazor-webassembly-app-with-azure-active-directory-aad-or-b2c"></a>Application hébergée Blazor WebAssembly avec Azure Active Directory (AAD) ou B2C

L' *`Client`* inscription de l’application d’une solution hébergée Blazor qui utilise AAD ou AAD B2C pour l’authentification des utilisateurs doit utiliser une **application à page unique** configuration de la plateforme Azure Apps :

1. Dans le Portail Azure l’inscription de l’application pour l' *`Client`* application, supprimez la configuration de la plateforme **Web** .
